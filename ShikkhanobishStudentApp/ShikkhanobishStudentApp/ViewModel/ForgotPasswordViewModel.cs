...
namespace ShikkhanobishStudentApp.ViewModel
{
    public class ForgotPasswordViewModel: BaseViewModel, INotifyPropertyChanged
    {
        ...
        bool hasmsgSend;
        string thispn;
        List<Student> allStudnetList = new List<Student>();
        ...
        {
            allStudnetList = await "https://api.shikkhanobish.com/api/ShikkhanobishLogin/getStudent".GetJsonAsync<List<Student>>();
        }
        #region Connectivity
        ...
        private async Task btn()
        {
            ...
            else if (clickCounter == 2)
            {
                ...
                {
                    await MaterialDialog.Instance.AlertAsync(message: "Password Doesn't Match",
                                    title: "Error!");
        ...
        public async Task checkPnumber()
        {
            ...
            if (pText1 != null && pText1 != "")
            {
                if (pText1.Length == 11)
                {
                    Student chkPn = new Student();
                    
                    using (var dialog = await MaterialDialog.Instance.LoadingDialogAsync(message: "Checking Phone Number..."))
                    {
                        for(int i = 0; i< allStudnetList.Count; i++)
                        {
                            if(allStudnetList[i].phonenumber == pText1)
                            {
                                thisStudent = allStudnetList[i];
                                chkPn = allStudnetList[i];
                                break;
                            }
                        }
                    }

                    if (chkPn.studentID == 0)
                    {
                        btnEnabled = false;
                        p1HasError = true;
